...
// Send email function with scheduling and bulk capabilities
const sendEmail = async ({
  ...
  try {
    ...
    const emailParams = new EmailParams()
      ...
      .setTo(recipients)
      .setReplyTo(sentFrom)
      .setCc(cc) // Add CC recipients
      .setBcc(bcc) // Add BCC recipients
      .setSubject(subject)
    ...
    // Bulk email array for multiple recipients
    const bulkEmail = []

    // Add email to bulk array
    bulkEmail.push(emailParams)

    // Send the email (scheduled or immediate)
    // For bulk sending, use:
    const data = await mailerSendInstance.email.sendBulk(bulkEmail)
...
export const MailerSendWithScheduleBulkProvider = {
  sendEmail
}